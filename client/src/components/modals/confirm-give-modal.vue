  ...
  <div class="modal-container">
    ...
    <transition name="modal-slideUp">
      <div class="modal-wrapper" v-show="beginAnimation">
        <div class="header">CONFIRM GIVE</div>
        <div class="content">
          <div class="top-text" style="margin-top: 10px">
            ...
            350 Coins and give these Invention Pieces away?
          </div>
          <div class="top-text" style="margin-bottom: 10px">This action cannot be undone.</div>
          ...
          <div class="opt-out">
            <input v-model="settings.modals.confirmGive" type="checkbox" id="optOut" value="optOut" />
            <label for="optOut"></label>
            <div
              class="opt-out-text"
            >Don't show this notification next time I want to go this action.</div>
          </div>
        </div>
      </div>
    </transition>
...
.header {
  position: absolute;
  top: calc(var(--inner-height) * (10 / 1920));
  left: 0;
  right: 0;
  width: 100%;
...
.invention-piece {
  ...
  padding-right: calc(var(--inner-height) * (15 / 1920));
}
...
@media screen and (min-device-width: 536px) {
  .yes-btn {
    width: calc(536px * (250 / 1080));
  }
  .no-btn {
    width: calc(536px * (250 / 1080));
  }
  .modal-wrapper {
    height: calc(536px * (945 / 1920));
    top: calc(536px * (300 / 1920));
  }
  .opt-out-text {
